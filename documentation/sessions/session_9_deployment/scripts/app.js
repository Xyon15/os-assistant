    ...
    setTimeout(function() {
        ...
        .then(donnees => {
            ...
            bouton.textContent = "Envoyer";
                ...
                .catch(erreur => {
            // 1. Supprimer le message de chargement (même code que dans .then)
            const messageChargement = document.getElementById("chargement");
            if (messageChargement) {
                messageChargement.remove();
            }
            
            // 2. Créer un message d'erreur poli (bulle rouge)
            const messageErreur = document.createElement("p");
            messageErreur.className = "message-error";  // Nouvelle classe CSS
            const bulleErreur = document.createElement("span");
            bulleErreur.className = "bulle-error";  // Nouvelle classe CSS
            ...
            // 3. Scroller vers le message d'erreur
            conversation.scrollTop = conversation.scrollHeight;
            
            // 4. Afficher erreur dans console (pour toi, développeur)
...
// === 4. FONCTON SUPPLÉMENTAIRE : EFFACER LA CONVERSATION ===
...
// === 5. FONCTION SUPPLÉMENTAIRE : BASCULER MODE SOMBRE ===
