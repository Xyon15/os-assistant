...
- sauvegarder_message(texte, nom_utilisateur, role) : Ajouter un message dans la db
...
def initialiser_db(): # Fonction qui permet de créer et préparer la db
    ...
    # Colonnes : id (auto), texte (message), nom_utilisateur (qui), role (user/assistant), date_creation (quand)
    connexion.execute("""
...
def sauvegarder_message(texte, nom_utilisateur, role="user"):  # Fonction pour ajouter un message dans la db
